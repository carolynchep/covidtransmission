...
class Room:
    ...
    def createRoom(cls, num_rows: int, num_cols: int) -> None:
        ...
        cls.entry = [Location((num_rows-1, num_cols//2), None, True), Location((num_rows//2, num_cols - 1), None, True), Location((0, num_cols // 2), None, True), Location((num_rows//2, 0), None, True) ]
        cls.locations[num_rows-1][num_cols//2] = cls.entry[0]
        cls.locations[num_rows//2][num_cols-1] = cls.entry[1]
        cls.locations[0][num_cols//2] = cls.entry[2]
        cls.locations[num_rows//2][0] = cls.entry[3]

        # convert the list-of-lists of locations to tuple-of-tuples f locations
    ...
    @classmethod
    def show(cls, t: int | float, clear_screen: bool = True, delay: float = 0.0) -> None:
    ...
    def arrival(cls, person: Person) -> None:
        ...
        '''
        random_entry = random.randint(0, 3)
        cls.entry[random_entry].occupant = person
        person.setLocation(cls.entry[random_entry])  # updates ._loc inside Person
    ...
    def isEntryOccupied(cls) -> bool:
        ...
        '''
        return bool(cls.entry[0].occupant and cls.entry[1].occupant and cls.entry[2].occupant and cls.entry[3].occupant)
    ...
    def getEntryCoords(cls) -> tuple[int,int]:
        ...
        '''
        return (cls.entry[0].row_col + cls.entry[1].row_col + cls.entry[2].row_col + cls.entry[3].row_col)
    ...
    @classmethod
    def getNeighbors(cls, person: Person, debug: bool = False) -> list[Person]:
        ''' method to return a list of occupied spaces
        Parameters:
            person: Person object considering movement
            debug: if True, prints debugging information
        Returns:
            a list of occupied locations of Person objects
        '''
        row = person.row()
        col = person.col()
        neighbor_list = []
        ...
        if row+1 < Room.rows:
            if cls.isLocationOccupied(row+1, col):
                neighbor_list.append(cls.locations[row+1][col].occupant)
        if col+1 < Room.cols:
            if cls.isLocationOccupied(row, col+1):
                neighbor_list.append(cls.locations[row][col+1].occupant)
        if row-1 >= 0:
            if cls.isLocationOccupied(row-1, col):
                neighbor_list.append(cls.locations[row-1][col].occupant)
        if col-1 >= 0:
